...
def draw_contour(shape):
	...
	cont = cv2.drawContours(face, [convexHull], -1, (255, 0, 0), 2)
...
print("Predictor and detector loaded")

'''
#FOR VIDEO

video_capture = cv2.VideoCapture(0)
model=load_model('efficientNetB0_JAFFE_binary_classification.h5')
model.summary()
filtered_face = np.zeros((1, 224, 224, 3), dtype=np.float32)
    ...
    if ret:
        ...
        for i, d in enumerate(dets):
            ...
            for i in range(0,len(shape)):
                ...
                resized_face = cv2.resize(filtered_face, (96, 96), interpolation = cv2.INTER_AREA)
