...
datapath = '../../CK_test' # dataset 593 neutras 327 não neutras
...
'''
#for all images add to the folder an image with high contrast and high brightness
#more bright
...
for label in labels:
    ...
    for img in img_list:
        ...
        zoom = cv2.resize(input_img, (0,0), fx=3, fy=3, interpolation=cv2.INTER_CUBIC)
        cv2.imwrite(datapath + '/'+ label + '/' +'zoom_10'+ img, zoom)
        #convert to gray
        ...
        cv2.imwrite(datapath + '/'+ label + '/'+'alpha_15_beta_10'+ img, input_img)

        input_img = cv2.resize(input_img, (0,0), fx=3, fy=3, interpolation=cv2.INTER_CUBIC)
        #save image to folder
        cv2.imwrite(datapath + '/'+ label + '/' +'zoom_11'+ img, input_img)
...
for label in labels:
    ...
    for img in img_list:
        ...
        input_img = cv2.convertScaleAbs(input_img, alpha=alpha, beta=beta)
        #save image to folder
        cv2.imwrite(datapath + '/'+ label + '/'+ 'alpha_05_beta_5'+ img, input_img)
        #
        input_img = cv2.resize(input_img, (0,0), fx=3, fy=3, interpolation=cv2.INTER_CUBIC)
        #save image to folder
        cv2.imwrite(datapath + '/'+ label + '/'+ 'zoom_21' + img, input_img)

#count number of images in each folder
for label in labels:
    img_list=os.listdir(datapath+'/'+ label+'/')
    print(label, len(img_list))
'''
for label in labels:
    print(label)
    img_list=os.listdir(datapath+'/'+ label+'/')
